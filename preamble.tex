...
\newcommand{\urbnlogo}[0]{
  \begin{figure}
    \hspace{-0.12in}
    \includegraphics[width=1.5in,clip]{design/urban_grid_blue_white.jpg}
    \label{fig:boat1}
    \vspace{-0.25in}
  \end{figure}
}
...
% footnote
\newcommand{\footnoteui}[0]{
  \thispagestyle{empty}
  \begin{tikzpicture}[remember picture, overlay]
   \draw
      node[align=center, anchor=south west, minimum height=0.5in, minimum width=8.5in, text width=8in] at ($(current page.south west) + (0in,0.38in)$)
      {
      \color{gray}{For more information on data and methods, see \href{https://www.urban.org/research/publication/health-insurance-coverage-women-reproductive-age-2013-2016}{\emph{Health Insurance Coverage for Women of Reproductive Age, 2017.}}}
      };
  \end{tikzpicture}
}
...
% figure source
\newcommand{\urbnsource}[2]{
\begin{spacing}{0.6}
  \textbf{\footnotesize{#1}} {\footnotesize{#2}}\break
  \end{spacing}
  \vspace{-0.3cm}
}

% figure note
\newcommand{\urbnnote}[2]{
\begin{spacing}{0.6}
  \textbf{\footnotesize{#1}} {\footnotesize{#2}}\break
  \end{spacing}
  \vspace{0.75cm}
...
{\begin{itemize}[leftmargin=*,labelindent=0in,labelsep=0.1875in]
...
\newcommand{\pastegraph}[0]{
  \begin{figure}
    \hspace{2in}
    \includegraphics[width=3in]{Plots/Alabama_plot.pdf}
    \label{fig:boat1}
  \end{figure}
}

\AtBeginDocument{\fontsize{10pt}{9pt}\selectfont}