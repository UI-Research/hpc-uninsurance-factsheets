...
state_table[,grep("_share",colnames(state_table))]<-format(round(as.numeric(state_table[,grep("_share",colnames(state_table))]*100),1),nsmall=1)
...
if (state_table$note == 0 & state_table$hier_fpw_share > 0) {
...
\\raggedright
\\urbnlogo{}
\\urbnfiguretitle{Figure 2. Potential Eligibility for Financial Assistance Obtaining Coverage among Uninsured Women of Reproductive Age in ", params$parameter1, " 2017}")
```
...
t$lab_for_figs_brks<-sapply(strwrap(t$lab_for_figs, 35, simplify=FALSE), paste, collapse="\n" )
 t2<-t %>% 
  mutate(labels_w_num = paste(lab_for_figs_brks, " \n",as.character(value), "%", sep =""), 
        ...
        # nudgex = ifelse(length(value)==3 & middle < pi, 3.5,
        #     ifelse(length(value)==3 & middle > pi,-3.5,
        #     ifelse(length(value)==4 & middle > pi*0.95 & middle < pi*1.3, -3.5, #if middle is close to pi, nudge towards -x
        #     ifelse(length(value)==4 & middle < pi*0.94, 3.5,
        #     ifelse(length(value)==4 & middle > pi*1.3,-3.5,0))))),  
