...
state_table[,grep("_mn",colnames(state_table))]<-as.numeric(state_table[,grep("_mn",colnames(state_table))])*100
...
state_table[,grep("_share",colnames(state_table))]<-as.numeric(state_table[,grep("_share",colnames(state_table))])*100

# apply formats to country_table
country_table[,grep("_mn",colnames(country_table))]<- as.numeric(country_table[,grep("_mn",colnames(country_table))])*100
country_table[,grep("_n",colnames(country_table))]<-as.numeric(country_table[,grep("_n",colnames(country_table))])
country_table[,grep("_tstat",colnames(country_table))]<-as.numeric(country_table[,grep("_tstat",colnames(country_table))])
...
pie_caption3<-ifelse(state_table$note == 4, "Some women likely eligible for Marketplace premium assistance may be eligible for more affordable coverage through the state's Basic Health Program. ", "")

# expasion states looking ahead text
if (state_table$expansion == 1 & state_table$note != 1 & state_table$note != 2 & state_table$note != 0) {
...
if (state_table$expansion == 0 & state_table$note == 0 & state_table$hier_fpw_share == 0 & state_table$hier_pov_none_share == 0){
  lookingahead<-paste("Expanding Medicaid in ",state_table$statename," could benefit approximately ", format(round(state_table$ui_mn, 1),nsmall=1), " percent of uninsured ", state_table$statename, " women of reproductive age, including currently ineligible citizens with incomes below the FPL (",format(round(state_table$hier_pov_none_share, 1), nsmall=1), " percent) and those with incomes between 100 and 138 percent of the FPL (",format(round(state_table$hier_100_138_share, 1), nsmall=1)," percent) for whom Medicaid may be more affordable than Marketplace coverage.",sep='')
}
...
\item Women of reproductive age in `r state_table$statename` had a `r statement2` uninsurance rate (`r format(round(state_table$ui_mn),nsmall=1)` percent) `r statement3` similar women nationwide (11.7 percent) in 2017 (figure 1).
...
t<-state_table %>% 
  ...
  left_join(pie_labs) %>% 
  filter(value>0) %>% 
  mutate(value = round(value, 1),nsmall=1) 
