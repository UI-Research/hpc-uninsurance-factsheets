...
state_plot <- total_data_for_chart %>%
  ...
  geom_text(aes(x = Label.for.Figures, y = mn_value, label=ifelse(location==as.character(state_table$statename) & mn_value>0,paste0(mn_value,"%"),'')), position = position_dodge(width = 1), hjust = -.25, size = 3) +
  theme(
  ...
  scale_fill_manual(labels = c("very negative", "kinda negative"))+
  coord_flip()
